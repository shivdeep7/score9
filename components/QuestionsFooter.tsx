...
const QuestionFooter = ({currentPage, disabled, submit = true}: {currentPage: string, disabled?: boolean, submit?: boolean}) => {
    ...
    return (
           <div className="flex justify-between items-center mt-5 absolute bottom-0 w-full p-3 bg-[#f6f4ee]">
            
            <div className="flex flex-row items-center space-x-3">
                {
                   ( currentPage!== "1") && (
                         <button className="p-3 px-5 bg-zinc-200  rounded-full  text-black font-[600] flex space-x-2" onClick={() => { router.push((Number(currentPage)-1).toString()) }} >
